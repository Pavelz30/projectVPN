...
echo -e "\e[0;34mЭтап $cur/$max. Создание запроса на сертификат и ключ для сервера.\e[0m"
./easyrsa --batch gen-req server nopass 
checkwork $cur
cur=$(( cur + 1 ))

echo -e "\e[0;34mЭтап $cur/$max Генерация tls-ключа для дополнительной безопасности.\e[0m"
...
if [ "$mode" == "along" ];
then
	echo -e "\e[0;34mЭтап $cur/$max. Подпись сертификата удостоверяющим центром.
	Здесь потребуется ввести пароль, заданный при генерации корневого сертефиката СA.\e[0m"
	./easyrsa --batch sign-req server server
	checkwork $cur
	cur=$(( cur + 1 ))

	echo -e "\e[0;34mЭтап $cur/$max. Копирование сертификата CA и сертификата сервера в /etc/openvpn/server/\e[0m"
...
echo -e "\e[0;34mЭтап $cur/$max. Копирование секретного ключа сервера и дополнительного ключа ta.key в /etc/openvpn/server.\e[0m"
sudo cp ./pki/private/server.key /etc/openvpn/server && \
	sudo cp ./ta.key /etc/openvpn/server/
checkwork $cur
cur=$(( cur + 1 ))

echo -e "\e[0;34mЭтап $cur/$cur Настройка openVPN и маршрутизации.\e[0m"
...
echo -e "\e[0;34mЭтап $cur/$max. Настройка iptables.\e[0m"
...
echo -e "\e[0;34mЭтап $cur/$max Запуск openVPN.\e[0m"
